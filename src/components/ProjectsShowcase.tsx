...
function ProjectCardGraphic({ project }: { project: Project }) {
  ...
  return (
    <div
      className={`relative w-full h-44 bg-linear-to-b ${bgGradient} border-b border-[#E6E1D3] p-5 flex flex-col justify-between overflow-hidden group-hover:bg-linear-to-t transition-all duration-300`}
    >
      ...
      <div className="flex justify-between items-start z-10">
        <span
          className={`px-2.5 py-1 rounded-full border text-[10px] font-mono uppercase tracking-wider ${accentColor} shadow-2xs`}
        >
          {project.category}
        </span>
      ...
      <div className="flex items-center justify-between z-10">
        ...
        {project.status ? (
          <span
            className={`px-2.5 py-1 rounded-full text-[10px] font-mono uppercase tracking-wider ${project.status === "IN PROGRESS" ? "bg-orange-50 text-orange-850 border border-orange-200" : "bg-white text-neutral-500 border border-[#E6E1D3]"}`}
          >
            {project.status}
          </span>
        ) : (
...
export default function ProjectsShowcase() {
  const [selectedCategory, setSelectedCategory] = useState<
    "all" | "web" | "mobile" | "design" | "dev-tool"
  >("all")
  const [activeProjectModal, setActiveProjectModal] = useState<Project | null>(
    null,
  )
    // Toggle to temporarily hide the entire Selected Projects section without deleting code.
    // Set to `true` to render the section again.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="mt-3 text-neutral-600 text-sm font-sans font-light leading-relaxed">
            Repos and live demos are landing here as I build them. Below is
            what's currently in progress and planned.
          </p>
        </div>
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div className="space-y-2.5">
                    <h4
                      ...
                      className="text-base font-display font-bold text-neutral-800 group-hover:text-orange-600 transition-colors"
                    >
                      {project.title}
                    </h4>
                ...
                <div className="bg-[#F4F1EA] border-b border-[#E6E1D3]/85 px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-xs font-mono text-neutral-600">
                      audit_log - {activeProjectModal.id}.json
                    </span>
                  </div>
                  <button
                    ...
                    <div className="md:col-span-8 space-y-6">
                      <div className="space-y-2">
                        ...
                        <h3
                          ...
                          className="text-2xl font-display font-extrabold text-[#2D2A26] tracking-tight pt-1"
                        >
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="space-y-3">
                          {activeProjectModal.details.map((bullet, i) => (
                            <li
                              key={i}
                              className="flex items-start space-x-2.5 text-xs text-neutral-700 font-sans leading-relaxed"
                            >
                      ...
                      <div className="space-y-3 pt-4 border-t border-[#E6E1D3]">
                        ...
                        <div className="space-y-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleLaunchSandbox(activeProjectModal.id)
                            }
                        ...
                        {sandboxActive === activeProjectModal.id && (
                          <div className="p-2 border border-emerald-200 bg-emerald-50 rounded-lg text-[9px] font-mono text-emerald-850 mt-2 leading-relaxed animate-fade-in">
                            🛠️ $ npm init virtual-sandbox --scope=
                            {activeProjectModal.id}
                            <br />
                            💡 connection established: 127.0.0.1:3000 to
                            container running!
                          </div>
                        )}
                      </div>
